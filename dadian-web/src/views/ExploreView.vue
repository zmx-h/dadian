...
import InvitationToast from '@/components/explore/InvitationToast.vue'
import { useLocationTracker } from '@/composables/useLocationTracker'

const explore = useExploreStore()
const { outing, route, missions, teammates, isPaused, teamSize } = storeToRefs(explore)
const { currentPosition, startTracking } = useLocationTracker()

const showCheckIn = ref(false)
const showTransfer = ref(false)
const showTeammates = ref(false)
const spots = ref<Spot[]>([])

const hasActiveOuting = computed(() => outing.value && outing.value.status !== 'completed')
const isRemote = computed(() => outing.value?.mode === 'remote' || false)
...
onMounted(async () => {
  ...
  pollTimer = setInterval(() => {
    if (hasActiveOuting.value) {
      explore.fetchTeammates().catch(() => {})
      explore.fetchPendingInvitations().catch(() => {})
    }
  }, 30000)
})
...
function toggleTeammates() {
  showTeammates.value = !showTeammates.value
}
...
<template>
  <div class="explore-page min-h-screen flex flex-col">
    ...
    <div class="flex-1 flex flex-col xl:flex-row gap-4 p-4 xl:p-6">
      ...
      <div class="xl:w-72 flex flex-col gap-4" :class="{ 'hidden xl:flex': !showTeammates }">
        <TeammatePanel :teammates="teammates" :is-remote="isRemote" />
    ...
    <BottomControlBar
      ...
      @toggle-teammates="toggleTeammates"
    />
